...
export default function Pronunciation() {
  ...
  async function getPhrase() {
    setTouched(true); // пользователь инициировал — можно показывать лимиты дальше
    ...
    try {
      ...
      if (r.status === 402 && d?.paywall) {
        ...
        // оставляем последнюю офлайн-фразу — не путаем пользователя пустым экраном
        return;
      }

      if (!r.ok) throw new Error(d?.error || `HTTP ${r.status}`);

      const phrase = String(d.phrase || "")
        .replace(/^["«»]+|["«»]+$/g, "")
        .trim();
      ...
      <div className="rounded-3xl p-6 md:p-10 bg-white/80 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 shadow grid md:grid-cols-[1.2fr,1fr] gap-6 items-center">
        <div>
          ...
          <p className="opacity-80 mt-2">
            Каждый раз — новая финская фраза уровня A1–A2.
          </p>
          ...
          {/* основной текст без упоминаний лимита до первого взаимодействия */}
          ...
          {/* после первой удачной загрузки — можно тонко показать оставшиеся попытки */}
        ...
        <div className="rounded-2xl bg-sky-50/60 dark:bg-slate-800/40 border border-sky-200/60 dark:border-slate-700 p-4 min-h-[160px]">
          {audioURL ? (
            ...
            <div className="h-24 rounded-xl bg-white/60 dark:bg-slate-9 00/40 grid place-items-center text-sm">
